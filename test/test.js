...
describe("Notes", () => {
  ...
  it("Create notes", (done) => {
    notes.forEach((note) => {
      chai.request(server).post("/notes").send(note).end((err, result) => {
        result.should.have.status(200);

        if (note == notes[notes.length - 1]) {
          chai.request(server).get("/notes/all").end((_err, _result) => {
  ...
  it("Fetch a list of all notes", (done) => {
    chai.request(server).get("/notes/all").end((err, result) => {
      result.should.have.status(200);
  ...
  it("Fetch a note", (done) => {
    createdNotes.forEach((note) => {
      chai.request(server).get("/notes/id/" + note.id).end((err, result) => {
        result.should.have.status(200);
  ...
  it("Fetch a user's notes", (done) => {
    ["eve", "adam", "team_10"].forEach((user) => {
      chai.request(server)
          .get("/notes/u/" + user)
          .end((err, result) => { result.should.have.status(200); });
    });
    done();
  });
      ...
      chai.request(server)
          ...
          .end((err, result) => {
            ...
            if (note.id == createdNotes[createdNotes.length - 1].id) {
              chai.request(server).get("/notes/all").end((_err, _result) => {
                _result.should.have.status(200);
                chai.assert.equal(_result.body.data.notes
  ...
  it("Delete notes", (done) => {
    createdNotes.forEach((note) => {
      chai.request(server).delete("/notes/" + note.id).end((err, result) => {
        result.should.have.status(200);
