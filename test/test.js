...
describe('User Accounts', () => {
  it('Fetch all registered users', (done) => { chai.request(server)
	  .get('/users')
	  .end((err, result) => {
  ...
  it('Create account', (done) => {
	chai.request(server)
	  .post('/signup')
	  .send({ email: 'test@account', password: '1234567890', username: 'test_user' })
	  .end((err, result) => {
		result.should.have.status(200)
		done()
	  })
  ...
  it('Block creation of multiple accounts with one email', (done) => {
	chai.request(server)
	  ...
	  .send({ email: 'test@account', password:'1234567890', username:'test_user' })
	  .end((err, result) => {
		chai.assert.equal(result.body.message, 'The email is already registered to a user')
		done()
	  })
  })
